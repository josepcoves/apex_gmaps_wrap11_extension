...
'                             p_min_columns    => 4,',
'                             p_max_columns    => 8, ',
'                             p_component_name => p_item.id); --Modified by JCoves (from 4 to 8)',
'    ',
'  ',
...
'      end if;',
'      if (t_column_value_list.EXISTS(8)) then',
'        t_marker_id          := t_column_value_list(8)(i);',
'      end if;',
'      --End JCoves',
'      htp.p(''          {'');',
'',
...
'         htp.p(''              group:"''||t_group||''",'');',
'      end if;',
'      if (t_marker_id is not null) then',
'         htp.p(''              id:"''||t_marker_id||''",'');',
'      end if;      ',
...
'      --End JCoves',
'      htp.p(''              html: {'');',
'      htp.p(''                content: ''''<div style="width: '' || /*to_char(to_number(p_item.element_width)-200) ||''px;''*/ ''auto;">'' || replace(/*do_subststring*/(t_markers_html_content), '''''''', ''\'''''') || ''</div>'''','');',
